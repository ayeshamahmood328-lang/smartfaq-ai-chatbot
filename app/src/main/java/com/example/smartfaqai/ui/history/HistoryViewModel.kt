...
class HistoryViewModel(private val repository: FaqRepository) : ViewModel() {
    val items: LiveData<List<SavedAnswerItem>> = MediatorLiveData<List<SavedAnswerItem>>().apply {
        addSource(repository.observeHistory()) { value = toConversations(it) }
    }

    fun delete(item: SavedAnswerItem) {
        viewModelScope.launch { repository.deleteConversation(item.key) }
    }
    ...
    private fun toConversations(messages: List<ChatMessageEntity>): List<SavedAnswerItem> =
